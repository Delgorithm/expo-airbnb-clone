...
CREATE TABLE `__new_wishlists` (
	...
	`created_at` integer DEFAULT 1748251191597,
	FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON UPDATE no action ON DELETE no action
);
--> statement-breakpoint
INSERT INTO `__new_wishlists`("id", "title", "location", "latitude", "longitude", "image", "user_id", "created_at") SELECT "id", "title", "location", "latitude", "longitude", "image", "user_id", "created_at" FROM `wishlists`;--> statement-breakpoint
DROP TABLE `wishlists`;--> statement-breakpoint
ALTER TABLE `__new_wishlists` RENAME TO `wishlists`;--> statement-breakpoint
...
CREATE TABLE `__new_listings` (
	...
	`longitude` real,
	`image` text,
	`created_at` integer DEFAULT 1748251191596
);
--> statement-breakpoint
